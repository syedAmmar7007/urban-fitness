        ...
        <div className="grid lg:grid-cols-2 gap-20">
          ...
          >
            ...
            <p className="text-gray-400 mb-12 leading-relaxed max-w-lg">
              Have questions about our classes or membership? Our team is here to help you start your fitness journey.
            </p>
            
            <div className="space-y-8">
              {[
                { icon: <MapPin />, label: "Our Location", value: "Plot 123, Block 5, Clifton, Karachi" },
                { icon: <Phone />, label: "Phone Number", value: "+92 300 1234567" },
                { icon: <Mail />, label: "Email Address", value: "info@urbanfitnesskhi.com" },
              ].map((item, idx) => (
                <motion.div 
                  key={idx}
                ...
                >
                  <div className="w-14 h-14 bg-white/5 flex items-center justify-center group-hover:bg-primary transition-colors duration-500">
                    {React.cloneElement(item.icon as React.ReactElement, { className: "text-primary group-hover:text-black" })}
                  </div>
                  <div>
                    <span className="text-xs font-bold text-gray-500 uppercase tracking-widest block mb-1">{item.label}</span>
          ...
          <motion.div 
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
            className="bg-white/[0.02] p-12 border border-white/5"
          >
